  ...
  function listeRender(bordrolar) {
    ...
    tbody.innerHTML = bordrolar.map(b => `
      <tr>
        ...
        <td style="font-size:13px;color:#22C55E;font-weight:600">${tl(b.brut_maas)}</td>
        <td>
        ...
        <td class="islem-td">
          ...
          <button class="btn-ikon" onclick="BordroModul.onayla(${b.id})" title="Onayla">
            <svg viewBox="0 0 20 20" fill="currentColor" style="color:#22C55E"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
          </button>` : ''}
          ${b.durum==='onaylandi' ? `
          <button class="btn-ikon" onclick="BordroModul.odendi(${b.id})" title="Ödendi İşaretle">
            <svg viewBox="0 0 20 20" fill="currentColor" style="color:#4F6EF7"><path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4z"/><path fill-rule="evenodd" d="M18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z" clip-rule="evenodd"/></svg>
          </button>` : ''}
          ${b.durum!=='odendi' ? `
    ...
    detayAc(id) {
      ...
      document.getElementById('bordro-modal-icerik').innerHTML = `
        <div class="detay-grid">
          ...
          <div class="detay-satir" style="font-weight:700"><span>Brüt Maaş</span><strong style="color:#22C55E">${tl(b.brut_maas)}</strong></div>
          <div class="detay-satir"><span>SGK İşçi (%14)</span><span style="color:#EF4444">- ${tl(b.sgk_isci)}</span></div>
          <div class="detay-satir"><span>İşsizlik (%1)</span><span style="color:#EF4444">- ${tl(b.issizlik_isci)}</span></div>
          <div class="detay-satir"><span>Gelir Vergisi</span><span style="color:#EF4444">- ${tl(b.gelir_vergisi)}</span></div>
          <div class="detay-satir"><span>Damga Vergisi</span><span style="color:#EF4444">- ${tl(b.damga_vergisi)}</span></div>
