...
const DemirbasModul = (() => {
  ...
  const DURUM_RENK = { depoda: '#656871', zimmetli: '#006CE0', bakimda: '#855900', hurda: '#8C8C94', kayip: '#DB0000' };
  ...
  function render() {
    ...
    O.icerik(`
      ${O.statGrid([
        ...
        { label: 'Zimmetli', deger: ozet.zimmetli, alt: 'personelde', renk: '#006CE0' },
        { label: 'Depoda', deger: ozet.depoda, alt: 'zimmete hazır', renk: '#656871' },
        { label: 'Bakım / Kayıp', deger: ozet.bakimda + ozet.kayip, alt: `${ozet.bakimda} bakımda · ${ozet.kayip} kayıp`, renk: (ozet.bakimda + ozet.kayip) ? '#855900' : '#00802F' },
      ])}
      ${O.sekmeler('dmb-tabs', [
        ...
        <table class="personel-tablo idari-tablo">
          ...
          <tbody>${liste.length ? liste.map(d => `
            <tr>
              ...
              <td>${O.tl(d.alis_bedeli)}${d.garanti_aktif ? '<span class="hucre-alt" style="color:#00802F">garantili</span>' : ''}</td>
              ...
              <td>${O.rozet(DURUM[d.durum] || d.durum, DURUM_RENK[d.durum] || '#656871')}</td>
              <td class="islem-td">
    ...
    async detay(id) {
      ...
      O.modalAc(`${d.kod} — ${d.ad}`, `
        <div class="detay-grid">
          ...
          <div class="detay-satir"><span>Durum</span>${O.rozet(DURUM[d.durum] || d.durum, DURUM_RENK[d.durum] || '#656871')}</div>
          ...
          <div class="detay-satir"><span>Garanti Bitiş</span><strong>${O.tarih(d.garanti_bitis)} ${d.garanti_aktif ? '<span style="color:#00802F">(aktif)</span>' : ''}</strong></div>
